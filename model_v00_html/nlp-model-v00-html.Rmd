...
Next word prediction given input text message is a sub-field of natural language processing. It is highly involved in text-based information propagation in social media embeded mostly on mobile devices, such as smartphone, laptop and tablet. [SwiftKey](https://swiftkey.com/en) is one of the leading company heading towards developing advanced machine learning technology to improve its customers' typing experience. Here, as a capstone project initiated by John Hopkins data science specialization and in corporation with SwiftKey, we also explored this area of research by building our own model predicting next word based on input text. Specifically, we use N-grams up to third order as the benchmark to construct our model. In addition, Good-Turing smoothing technique and Katz back-off method are used to improve our prediction accurary.

## Data loading and cleaning

We load a sample corpus of txt documents composed of blogs, news and tweets provided by swiftkey in the data science capstone project. We used __tm__ package to manage documents, which are loaded from a function called __Corpus__. In order to save memory, __PCorpus__ (named permanent corpus) is used, which essentially creates pointers to external data base and manipulates underlying corpus. Several examples of individual entries in the corpus are shown below:
...
```{r load corpus, message=FALSE, echo=TRUE}
library(tm)
library(filehash)
fname <- "../../final/en_US/"
...
```{r cleaned corpus, message=FALSE, echo=TRUE}
...
```{r eda, message=FALSE, results='asis', echo=TRUE}
library(tau)
library(xtable)
...
print(xtable(summaryCorpus, digits = 4), type="html", comment=FALSE)
```
...
```{r unigram, message=FALSE, eval=FALSE, echo=TRUE}
...
```{r smoothing, message=FALSE, results='asis', echo=FALSE}
dirtrialngram <- "../trial-ngram/"
unigram.fft <- read.csv(file = paste(dirtrialngram, "unigram-fft.csv", sep = ""))
bigram.fft  <- read.csv(file = paste(dirtrialngram, "bigram-fft.csv" , sep = ""))
trigram.fft <- read.csv(file = paste(dirtrialngram, "trigram-fft.csv", sep = ""))
...
print(xtable(tableGT), type = "html", comment=FALSE)
```
...
```{r predict-eg, message=FALSE, results='asis', echo=TRUE}
library(xtable)
print(xtable(nextWordPrediction("consider the following")), type = "html", comment=FALSE)
print(xtable(nextWordPrediction("this is made in")), type = "html", comment=FALSE)
print(xtable(nextWordPrediction("I'm running out of")), type = "html", comment=FALSE)
```
