...
nline = 10000
nstep = 10
list.train <- list('blogs' = docs.train[[1]][[1]][1:(nline*nstep)],
                   'news'  = docs.train[[2]][[1]][1:(nline*nstep)],
                   'tweet' = docs.train[[3]][[1]][1:(nline*nstep)])
```
...
lnword <- nchar(unigram.wf$words) > 12
...
# index individual words by numbers
unigram.wf$index <- 1:nrow(unigram.wf)
# write to folder
...
```{r ngram-clean-fn, eval=FALSE, echo=FALSE}
# clear abnormal words in ngrams
NgramClean <- function(ngram.wf) {
      # find ngram with too-long words
      lnword <- sapply(1:nrow(ngram.wf), function(i) {
            word <- ngram.wf$words[i]; word.list <- strsplit(word, split = " ");
            word.list <- unlist(word.list); max(nchar(word.list)) > 12})
      ngram.wf <- ngram.wf[!lnword, ]
      # bigrams with <num>, <eos>, <url>, <email>
      spword <- grepl("<(.*)+>", ngram.wf$words)
      ngram.wf <- ngram.wf[!spword, ]
      # long duplicate pattern, e.g. aaahhhhhh, ahahah, loooook
      # grep of this pattern is hard to deal with long word string
      dpword <- grepl("([a-z]+)+\\1{2,}", ngram.wf$words)
      ngram.wf <- ngram.wf[!dpword, ]
      # words start with apostrophe, e.g. 'stoke
      apword <- grepl("^\\'|[[:space:]]\\'", ngram.wf$words)
      ngram.wf <- ngram.wf[!apword, ]
      # deal with concatenate sign (i.e. -), all should be removed except concatenated words
      cnword <- grepl("^([a-z]+-)+[a-z]+$", ngram.wf$words)
      cnsign <- grepl("-", ngram.wf$words)
      ngram.wf <- ngram.wf[!(cnsign & !cnword), ]
      # more than one punctuation in ngram
      mpunct <- grepl("[[:punct:]][[:space:]][[:punct:]]", ngram.wf$words)
      ngram.wf <- ngram.wf[!mpunct, ]
      return(ngram.wf)
}
```
...
# step 1: generate all bigrams within chunks
if(!dir.exists("tmp/")) dir.create("tmp/")
for(list.name in names(list.train)) {
      corp <- list.train[[list.name]]
      for(i in 1:nstep) {
            print(paste("Bigram iteration", list.name, i, "of", nstep))
            chunk <- corp[1:nline]
            bigram <- ngram(chunk, 2)
            bigram.wf <- data.frame(words = names(bigram), freq = unclass(bigram),
                                    stringsAsFactors = FALSE)
            bigram.wf <- NgramClean(bigram.wf)
            fname <- paste0("bigram-wf-", list.name, i, ".csv")
            write.csv(bigram.wf, paste0("tmp/", fname), row.names = FALSE)
            corp <- corp[-(1:nline)]
      }
      rm(corp, chunk, bigram, bigram.wf)
}
# step 2: 1st step aggregating
# merged files according to literature types
for(list.name in names(list.train)) {
      print(paste("Merge bigrams in", list.name))
      fname <- paste0("tmp/bigram-wf-", list.name, "1.csv")
      bigram.wf <- read.csv(fname, stringsAsFactors = FALSE)
      colnames(bigram.wf) <- c("words", "freq1")
      for(i in 1:(nstep-1)) {
            fname <- paste0("tmp/bigram-wf-", list.name, (i+1), ".csv")
            tmpbigram.wf <- read.csv(fname, stringsAsFactors = FALSE)
            colnames(tmpbigram.wf) <- c("words", paste0("freq",(i+1)))
            bigram.wf <- merge(bigram.wf, tmpbigram.wf, by = "words", 
                               suffixes = NULL, all = TRUE)
            rm(tmpbigram.wf)
      }
      outfile <- paste0("tmp/mergedBi-", list.name, ".csv")
      bigram.wf$freq <- rowSums(bigram.wf[, 2:ncol(bigram.wf)], na.rm = TRUE)
      bigram.wf <- bigram.wf[, -(2:(ncol(bigram.wf)-1))]
      write.csv(bigram.wf, outfile, row.names = FALSE)
      rm(bigram.wf)
}
# step 3: final merge
print("Final bigram merge")
fname <- paste0("tmp/mergedBi-", names(list.train)[1], ".csv")
bigram.wf <- read.csv(fname, stringsAsFactors = FALSE)
colnames(bigram.wf) <- c("words", paste0("freq-", names(list.train)[1]))
for(list.name in names(list.train)[2:3]) {
      fname <- paste0("tmp/mergedBi-", list.name, ".csv")
      tmpbigram.wf <- read.csv(fname, stringsAsFactors = FALSE)
      colnames(tmpbigram.wf) <- c("words", paste0("freq-", list.name))
      bigram.wf <- merge(bigram.wf, tmpbigram.wf, by = "words", 
                         suffixes = NULL, all = TRUE)
      rm(tmpbigram.wf)
}
...
rm(bigram.fft)
# indexing bigrams
bigram.wf$w1 <- gsub(" .*", "", bigram.wf$words)
bigram.wf$w2 <- gsub(".* ", "", bigram.wf$words)
bigram.wf <- bigram.wf[,-1]
unigram.wf <- read.csv("ngram/unigram-wf.csv", stringsAsFactors = FALSE)
bigram.wf <- merge(bigram.wf, unigram.wf[,c("words", "index")],
                   by.x = "w1", by.y = "words", suffixes = NULL, all = TRUE)
setnames(bigram.wf, "index", "ind1")
bigram.wf <- merge(bigram.wf, unigram.wf[,c("words", "index")],
                   by.x = "w2", by.y = "words", suffixes = NULL, all = TRUE)
setnames(bigram.wf, "index", "ind2")
bigram.wf <- bigram.wf[,c("ind1", "ind2", "freq")] %>% arrange(desc(freq))
# impute rows with NA's
bigram.wf <- bigram.wf[complete.cases(bigram.wf),]
# write to folder
if(!dir.exists("ngram/")) dir.create("ngram/")
write.csv(bigram.wf, "ngram/bigram-wf.csv", row.names = FALSE)
rm(bigram.wf, unigram.wf)
```

```{r trigram, eval=FALSE, echo=FALSE}
# step 1: generate all trigrams within chunks
if(!dir.exists("tmp/")) dir.create("tmp/")
for(list.name in names(list.train)) {
      corp <- list.train[[list.name]]
      for(i in 1:nstep) {
            print(paste("Trigram iteration", list.name, i, "of", nstep))
            chunk <- corp[1:nline]
            trigram <- ngram(chunk, 3)
            trigram.wf <- data.frame(words = names(trigram), freq = unclass(trigram),
                                    stringsAsFactors = FALSE)
            trigram.wf <- NgramClean(trigram.wf)
            fname <- paste0("trigram-wf-", list.name, i, ".csv")
            write.csv(trigram.wf, paste0("tmp/", fname), row.names = FALSE)
            corp <- corp[-(1:nline)]
      }
      rm(corp, chunk, trigram, trigram.wf)
}
# step 2: 1st step aggregating
# merged files according to literature types
for(list.name in names(list.train)) {
      print(paste("Merge trigrams in", list.name))
      fname <- paste0("tmp/trigram-wf-", list.name, "1.csv")
      trigram.wf <- read.csv(fname, stringsAsFactors = FALSE)
      setnames(trigram.wf, "freq", "freq1")
      for(i in 1:(nstep-1)) {
            fname <- paste0("tmp/trigram-wf-", list.name, (i+1), ".csv")
            tmptrigram.wf <- read.csv(fname, stringsAsFactors = FALSE)
            setnames(tmptrigram.wf, "freq", paste0("freq",(i+1)))
            trigram.wf <- merge(trigram.wf, tmptrigram.wf, by = "words", 
                               suffixes = NULL, all = TRUE)
            rm(tmptrigram.wf)
      }
      outfile <- paste0("tmp/mergedTri-", list.name, ".csv")
      trigram.wf$freq <- rowSums(trigram.wf[, 2:ncol(trigram.wf)], na.rm = TRUE)
      trigram.wf <- trigram.wf[, -(2:(ncol(trigram.wf)-1))]
      write.csv(trigram.wf, outfile, row.names = FALSE)
      rm(trigram.wf)
}
# step 3: final merge
print("Final trigram merge")
fname <- paste0("tmp/mergedTri-", names(list.train)[1], ".csv")
trigram.wf <- read.csv(fname, stringsAsFactors = FALSE)
setnames(trigram.wf, "freq", paste0("freq-", names(list.train)[1]))
for(list.name in names(list.train)[2:3]) {
      fname <- paste0("tmp/mergedTri-", list.name, ".csv")
      tmptrigram.wf <- read.csv(fname, stringsAsFactors = FALSE)
      setnames(tmptrigram.wf, "freq", paste0("freq-", list.name))
      trigram.wf <- merge(trigram.wf, tmptrigram.wf, by = "words", 
                         suffixes = NULL, all = TRUE)
      rm(tmptrigram.wf)
}
...
write.csv(trigram.fft, "ngram/trigram-fft.csv", row.names = FALSE)
rm(trigram.fft)
# indexing trigrams
biword <- gsub(" [^ ]+$", "", trigram.wf$words)
trigram.wf$w1 <- gsub(" .*", "", biword)
trigram.wf$w2 <- gsub(".* ", "", biword)
rm(biword)
trigram.wf$w3 <- gsub(".* ", "", trigram.wf$words)
trigram.wf <- trigram.wf[,-1]
unigram.wf <- read.csv("ngram/unigram-wf.csv", stringsAsFactors = FALSE)
trigram.wf <- merge(trigram.wf, unigram.wf[,c("words", "index")], 
                    by.x = "w1", by.y = "words", suffixes = NULL, all = TRUE)
setnames(trigram.wf, "index", "ind1")
trigram.wf <- merge(trigram.wf, unigram.wf[,c("words", "index")], 
                    by.x = "w2", by.y = "words", suffixes = NULL, all = TRUE)
setnames(trigram.wf, "index", "ind2")
trigram.wf <- merge(trigram.wf, unigram.wf[,c("words", "index")], 
                    by.x = "w3", by.y = "words", suffixes = NULL, all = TRUE)
setnames(trigram.wf, "index", "ind3")
trigram.wf <- trigram.wf[,c("ind1","ind2","ind3","freq")] %>% arrange(desc(freq))
# impute rows with NA's
trigram.wf <- trigram.wf[complete.cases(trigram.wf),]
# write to folder
if(!dir.exists("ngram/")) dir.create("ngram/")
write.csv(trigram.wf, "ngram/trigram-wf.csv", row.names = FALSE)
rm(trigram.wf, unigram.wf)
```
...
dirngram <- "ngram/"
unigram.fft <- read.csv(file = paste0(dirngram, "unigram-fft.csv"))
bigram.fft  <- read.csv(file = paste0(dirngram, "bigram-fft.csv" ))
trigram.fft <- read.csv(file = paste0(dirngram, "trigram-fft.csv"))
# scatter plot
library(scales)
par(mfrow = c(1,3))
...
```{r final-input-data, echo=FALSE}
# prepare bigram and trigram word frame for future manipulation
#
trigram.wf <- fread("ngram/trigram-wf.csv", key = "freq")
trigram.fft <- fread("ngram/trigram-fft.csv", key = "freq")
trigram.wf <- merge(trigram.wf, trigram.fft[,c(1,3)])
# trigram leftover probability
trigram.beta <- trigram.wf[, .(probLeftover = probLeftover(freq, discount)), 
                           by = ind1:ind2]
bigram.wf <- fread("ngram/bigram-wf.csv", key = "freq")
bigram.fft <- fread("ngram/bigram-fft.csv", key = "freq")
bigram.wf <- merge(bigram.wf, bigram.fft[,c(1,3)])
# bigram leftover probability
bigram.beta <- bigram.wf[, .(probLeftover = probLeftover(freq, discount)), 
                         by = ind1]
unigram.wf <- fread("ngram/unigram-wf.csv", key = "freq")
unigram.fft <- fread("ngram/unigram-fft.csv", key = "freq")
unigram.wf <- merge(unigram.wf, unigram.fft[,c(1,3)])
# write to folder
if(!dir.exists("input/")) dir.create("input/")
fwrite(trigram.wf, "input/trigram-wf.csv")
fwrite(trigram.beta, "input/trigram-beta.csv")
fwrite(bigram.wf, "input/bigram-wf.csv")
fwrite(bigram.beta, "input/bigram-beta.csv")
fwrite(unigram.wf, "input/unigram-wf.csv")
rm(unigram.wf, unigram.fft, bigram.wf, bigram.fft, bigram.beta, 
   trigram.wf, trigram.fft, trigram.beta)
```
